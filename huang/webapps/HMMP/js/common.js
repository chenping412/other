...
$('.filter-bar input').click(function (event) {
 ...
 * @returns
 */
function initAgentAutocomplete(callback) {
	$.ajax({
		type : 'POST',
		dataType : 'json',
 ...
 * @returns
 */
function initRoomAutocomplete(hotelId,callback) {
    $.ajax({
        type : 'POST',
        dataType : 'json',
        contentType : 'application/json; charset=UTF-8',
        context : document.body,
        url : projectPath + 'admin/queryAutoCompleteRoomList',
        data:hotelId,
        success : function(data) {
            if (data.result && data.result == 1) {
                initAutocomplete(JSON.stringify(data.obj), 'roomTypeName',
                    'roomTypeId',callback);
            } else {
                new Tip({
                    msg : data.errorReason,
 ...
 * @returns
 */
function initUserAutocomplete(callback) {
    $.ajax({
        type : 'POST',
        dataType : 'json',
        contentType : 'application/json; charset=UTF-8',
        context : document.body,
        url : projectPath + 'admin/userAutocomplete',
        success : function(data) {
            if (data.result && data.result == 1) {
                initAutocomplete(JSON.stringify(data.obj), 'maker',
                    'makerId',callback);
            } else {
                new Tip({
                    msg : data.errorReason,
...
* ajaxForm提交
*/
function ajaxFormSubmit(formId,url,target,callbackFunction,postType,dataType){
    if(!postType){
        postType="post";
    }
    var options = {
