...
<%--                     </shiro:hasPermission> --%>
                    ...
                    <%--<td><a href="##" class="addbtn add_room">新增房型</a></td>--%>
                    <%--</shiro:hasPermission>--%>
                    <%--<shiro:hasPermission name="product:hotel:product:add">--%>
                    <%--<td><a href="##" class="addbtn add_priceplan">新增产品</a></td>--%>
                    <%--</shiro:hasPermission>--%>
                    <%--<shiro:hasPermission name="product:hotel:additional:add">--%>
                    <%--<td><a href="##" class="addbtn add_addtional">新增杂费</a></td>--%>
                    <%--</shiro:hasPermission>--%>
                </tr>
            </table>
        </div>
        ...
        <form id="hotelInfoForm" action="queryHotelList" method="post">
                            ...
                            <li style="width: 24%; text-align:  right; color: #FF6000;">
                                <div class="table-list-content-tools">
                                    <ul class="clearfix">
                                        ...
                                        <li><a href="javascript:queryRoomTypeList('${hotel.hotelId}','${hotel.hotelName}');" class="check-btn">房型</a><span>|</span></li>
                                        </shiro:hasPermission>

<%--                                         <shiro:hasPermission name="product:hotel:product:query"> --%>
<%--                                         <li><a href="javascript:queryPricePlanList('${hotel.hotelId}','${hotel.hotelName}');">产品</a><span>|</span></li> --%>
<%--                                         </shiro:hasPermission> --%>

<%--                                         <shiro:hasPermission name="product:hotel:additional:query"> --%>
<%--                                         <li><a href="javascript:queryAdditionalList('${hotel.hotelId}','${hotel.hotelName}');">杂费</a><span>|</span></li> --%>
<%--                                         </shiro:hasPermission> --%>
...
<script>
    ...
    $('.add_hotel').on('click', function () {
        $(window.parent.document).find(".product-manage").children().attr('src','<%=basePath%>admin/toAddHotel');
    });

    $('.add_room').on('click', function () {
        $(window.parent.document).find(".product-manage").children().attr('src','<%=basePath%>admin/toAddRoom');
    });

    $('.add_priceplan').on('click', function () {
        $(window.parent.document).find(".product-manage").children().attr('src','<%=basePath%>admin/toAddPricePlan');
    });


    $('.add_addtional').on('click', function () {
        $(window.parent.document).find(".product-manage").children().attr('src','<%=basePath%>admin/toAddAdditional');
    });
    ...
    function delHotel(hotelId){
        console.log("delHotel:hotelID="+hotelId);
        $.ajax({
            type: 'POST',
            ...
            success: function(rs) {
                $('.f_ui-loading').hide();
                if (rs.result == 1) {
                    new Tip({ msg: "删除成功！",timer: 100000, type:1});
                    queryHotelInfoList();
                } else {
    ...
    // 查看房型列表
    function queryRoomTypeList(hotelId,hotelName){
    ...
    function queryPricePlanList (hotelId,hotelName) {
        $(window.parent.document).find(".product-manage").children().attr('src','<%=basePath%>admin/queryPricePlanList?hotelId='+hotelId+'&hotelName='+hotelName);
        <%--$(window.parent.document).find(".product-manage").children().attr('src','<%=basePath%>admin/toBatchSetPriceAndQuota');--%>
    }

    function queryAdditionalList (hotelId,hotelName) {
        $(window.parent.document).find(".product-manage").children().attr('src','<%=basePath%>admin/queryAdditionalList?hotelId='+hotelId+'&hotelName='+hotelName);
        <%--$(window.parent.document).find(".product-manage").children().attr('src','<%=basePath%>admin/toBatchSetPriceAndQuota');--%>
    }
</script>