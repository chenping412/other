...
<div class="add-role-wrap">
    ...
    <div class="main-content">
        <div class="section-wrap section-wrap-function">
            <div class="section-content">
                ...
                <form action="#" class="form-list">
                    ...
                    <ul class="row">
                        <li class="list"><span>角色名称：</span><input class="role-name" type="text" id="roleName" name="roleName" value="${role.roleName}"  placeholder="请输入角色名称"><i class="empty iconfont icon-ArtboardCopy"></i></li>
                        <li class="list"><span>角色编码：</span><input class="role-code" type="text" id="roleCode" name="roleCode" value="${role.roleCode}" placeholder="请输入角色编码" readonly><i class="empty iconfont icon-ArtboardCopy"></i></li>
                        <li class="list"><span>角色描述：</span><input class="role-des" type="text" id="description" name="description" value="${role.description}" placeholder="请输入角色描述"><i class="empty iconfont icon-ArtboardCopy"></i></li>
                    </ul>
                </form>
            </div>
        </div>
        ...
        <div class="section-wrap section-wrap-function">
            <div class="section-content">
                <h3>2.设置权限</h3>
                <input type="hidden" id="currentRoleResourceIds" name="currentRoleResourceIds" value="${currentRoleResourceIds}">
        ...
        <div class="search-footer">
            <div style="float:right">
                ...
                <a href="javascript:editRoleInfo();" class="save" >保存</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(function () {
        var currentRoleResourceIds = $("#currentRoleResourceIds").val();
        var jsDatas= eval(currentRoleResourceIds);

        for ( var i = 0; i < jsDatas.length; i++) {
            $('.select-tree-left').find('.select-tree-first-layer').each(function(){
                // 一级权限，没有子权限的，直接选中
                if($(this).data('layer').substr(1) == jsDatas[i] && $(this).find('.select-tree-second-layer').length <= 0){
                    $(this).find('.check-all').trigger("click");
                }
            });
            $('.select-tree-left').find('.select-tree-second-layer').each(function(){
                // 二级权限，没有子权限的，直接选中
                if($(this).data('layer').substr(1) == jsDatas[i] && $(this).find('.select-tree-third-layer').length <= 0){
                    $(this).find('.check-second-floor').trigger("click");
                }
            });
            $('.select-tree-left').find('.select-tree-third-layer').each(function(){
                if($(this).data('layer').substr(1) == jsDatas[i]){
                    $(this).find('.check-third-floor').trigger("click");
                }
            });
        }
    });
    ...
    function editRoleInfo() {
        ...
        $.ajax({
            ...
            url : '<%=basePath%>admin/editRoleInfo',
            data:{
                "id":$("#id").val(),
                "roleName":roleName,
