...
<body>
	<div id="uploader" class="wu-example">
		...
		<input name="attachmentType" type="hidden" value="${attachmentType}" />
...
uploader = WebUploader.create({
    ...
    // 文件接收服务端。
    server: '<%=basePath%>admin/orgUpload',
    // 选择文件的按钮。可选。
    // 内部根据当前运行是创建，可能是input元素，也可能是flash.
    pick: '#picker',
    formData: {
    	businessId:$('input[name="orgCode"]').val(),
    	attachmentType:$('input[name="attachmentType"]').val()
    },
    accept: {
    	title: 'Applications',
    	extensions: 'doc,docx,pdf,jpg,jpeg,tiff',
    	mimeTypes: 'application/msword,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,image/jpeg,image/tiff'
    },
    fileNumLimit: 3,
...
uploader.on('error', function( type ){
	...
	if ( type === "Q_TYPE_DENIED" ){
		new Tip({msg : '不支持的附件格式',timer : 3000,type : 2});
	}
