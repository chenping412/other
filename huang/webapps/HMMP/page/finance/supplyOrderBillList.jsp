...
<div class="finance-manage-wrap">
	<div class="main-content">
		...
		<div class="section-wrap section-wrap-function">
			...
			<table>
				<tr>
					<td><a href="##" class="addbtn" onclick="batchCheckOut('supplyBillController');">批量标记对账</a></td>
					<td><a href="##" class="addbtn addSupplyBillOff">销账</a></td>
					<td><a id="supplyBillExportPDF" href="##" class="addbtn">导出PDF</a></td>
					<td><a id="supplyBillExportExcel" href="##" class="addbtn">导出Excel</a></td>
				</tr>
			</table>
		</div>
		<br/>
		...
		<form id="queryOrderBillListForm" method="post">
			...
			<input type="hidden" name="beginDate" id="beginDate" value="${supplyCheckOutQuery.beginDate}" />
			<input type="hidden" name="endDate" id="endDate" value="${supplyCheckOutQuery.endDate}"/>

			<input type="hidden" id="payMethod" name="payMethod" value="${supplyCheckOutQuery.payMethod}"/>
			...
			<input type="hidden" id="supplyName" name="supplyName" value="${supplyCheckOutQuery.supplyName}"/>

			<input type="hidden" placeholder="" name="guestName" id="guestName" value="${supplyCheckOutQuery.guestName}"/>
			...
			<input type="hidden" placeholder="" name="orderCode" id="orderCode" value="${supplyCheckOutQuery.orderCode}"/>
		...
		<div class="table-list">
			<div class="table-list-header" style="width:100%;">
				<ul class="clearfix">
					...
					<li style="width: 5%; text-align: center;">底价</li>
					<li style="width: 5%; text-align: center;">代付价</li>
					<c:if test="${supplyCheckOutQuery.payType==1}">
						<li style="width: 5%; text-align: center;">应付</li>
						<li style="width: 5%; text-align: center;">已付</li>
						<li style="width: 5%; text-align: center;">本次付</li>
					</c:if>
					...
					<c:forEach items="${recordList}" var="dayCheckOut"	varStatus="status">
						<c:if test="${not empty dayCheckOut}">
							<c:forEach items="${dayCheckOut.orgCheckOutDetailList}" var="detail"	varStatus="status">
								<ul class="row clearfix">
									...
									<li style="width: 5%; text-align: center;">${detail.basePrice}</li>
									...
									<li style="width: 3%">${detail.currency}
										<input type="hidden" id="baseCurrency_${detail.dayPriceId}" value="${detail.currency}" />
									</li>
									...
									<input type="hidden" value="${detail.supplyOrderBillStatus}" id="supplyOrderBillStatus_${detail.dayPriceId}">
									<c:choose>
										<c:when test="${empty detail.checkOutId}">
											<li style="width: 7%; color: #FF6000;text-align: center;" id="label_checkout_${detail.dayPriceId}">${detail.supplyBillUser}</li>
										</c:when>
										<c:otherwise>
											<li style="cursor:pointer;width: 7%; color: #009BF1;text-align: center;" id="label_checkout_${detail.dayPriceId}" onclick="checkOut('${detail.dayPriceId}','${detail.dayPriceId}','supplyBillController')">标记对账</li>
										</c:otherwise>
									</c:choose>
									<li style="width: 3%; text-align: center;color: #FF6000;">
						...
						<ul class="row clearfix">
							...
							<li style="width: 16%" class="font-bold">
								底价：<c:forEach items="${dayCheckOut.dayCheckOutList}" var="detail"	varStatus="status">${detail.basePrice}${detail.currency}&nbsp;</c:forEach>
							</li>
							<li style="width: 16%" class="font-bold">
								代付价：<c:forEach items="${dayCheckOut.dayCheckOutList}" var="detail"	varStatus="status">${detail.saleCPrice}${detail.currency}&nbsp;</c:forEach>
							</li>
							<li style="width: 16%" class="font-bold">
								<c:if test="${supplyCheckOutQuery.payType==1}">应付：</c:if>
								<c:if test="${supplyCheckOutQuery.payType==0}">应收：</c:if>
								<c:forEach items="${dayCheckOut.dayCheckOutList}" var="detail"	varStatus="status">${detail.receivable}${detail.currency}&nbsp;</c:forEach>
							</li>
							<li style="width: 16%" class="font-bold">
								<c:if test="${supplyCheckOutQuery.payType==1}">已付：</c:if>
								<c:if test="${supplyCheckOutQuery.payType==0}">已收：</c:if>
								<c:forEach items="${dayCheckOut.dayCheckOutList}" var="detail"	varStatus="status">${detail.actualReceived}${detail.currency}&nbsp;</c:forEach>
							</li>
							<li style="width: 16%" class="font-bold">
								<c:if test="${supplyCheckOutQuery.payType==1}">未付：</c:if>
								<c:if test="${supplyCheckOutQuery.payType==0}">未收：</c:if>
								<c:forEach items="${dayCheckOut.dayCheckOutList}" var="detail"	varStatus="status">${detail.unreceivable}${detail.currency}&nbsp;</c:forEach>
							</li>
							<li style="width: 1%"></li>
						</ul>
					</c:forEach>
...
<script>
	...
	$(function(){
		...
		$(".addSupplyBillOff").on("click",function () {
			...
			$("input[name='dayPriceId']:checked").each(function() {
				...
				var billStatus = $("#supplyOrderBillStatus_"+rowNum).val();
				var checkOuter = $("#label_checkout_"+rowNum).html();
				var currency = $('#baseCurrency_'+rowNum).val();
			...
			$.ajax({
				...
				url : '<%=basePath%>admin/supplyBillController/toAddBillOff',
				data : data,
		...
		$("#supplyBillExportPDF").on("click",function () {
			var form = getQueryParam(projectPath + 'admin/exportSupplyBillPDF');
			if (form) {
				form.submit();
				form.remove();
			}
		});
		$("#supplyBillExportExcel").on("click",function () {
			var form = getQueryParam(projectPath + 'admin/exportSupplyBillExcel');
			if (form) {
				form.submit();
				form.remove();
			}
...
//		form.append(agentCode);
	...
	function billOff() {
		...
		var orgCode = $("#supplyCode").val();
		var orgName = $("#supplyName").val();
		...
		if(null == serialNumber || serialNumber == ""){
			new Tip({msg : "销账凭号为空",timer : 1000,type : 2});
			return false;
		}
		...
		$.ajax({
			...
			context : document.body,
			url : '<%=basePath%>admin/supplyBillController/billOff',
