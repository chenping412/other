...
<body>
	<div class="finance-manage-wrap">
		...
		<div class="main-content">
			<shiro:hasPermission name="finance:orderAppeal:add">
			<div class="section-wrap section-wrap-function">
				<div class="section-content">
					<a href="<%=basePath%>admin/orderAppealController/addPage"
						class="addbtn">发起申诉</a>
				</div>
			</div>
			</shiro:hasPermission>
			<shiro:hasPermission name="finance:orderAppeal:query">
			...
			<div class="table-list">
				<div class="table-list-header">
					<ul class="clearfix">
						...
						<li style="width: 10%">申诉时间</li>
						<li style="width: 10%">处理人</li>
						<li style="width: 10%">处理时间</li>
						<li style="width: 10%">申诉状态</li>
						<li style="width: 13%">备注</li>
					...
					<c:if test="${not empty page}">
						<c:forEach items="${page.recordList}" var="orderAppeal"
							...
							<ul class="row clearfix">
								...
								<li class="list99" style="width: 10%"><fmt:formatDate value="${orderAppeal.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></li>
								<li style="width: 10%">${orderAppeal.handler}</li>
								<li class="list99" style="width: 10%"><fmt:formatDate value="${orderAppeal.handleTime}" pattern="yyyy-MM-dd HH:mm:ss"/></li>
								<li style="width: 10%">${orderAppeal.statusText}</li>
								<li class="list100" style="width: 13%">${orderAppeal.mark}</li>
								<li style="width: 10%;color:#FF6000">
									<div class="table-list-content-tools">
										<ul class="clearfix">
											<shiro:hasPermission name="finance:orderAppeal:edit">
												<c:if test="${orderAppeal.status == 'new'}">
													<li><a href="<%=basePath%>admin/orderAppealController/editPage/${orderAppeal.appealId}">编辑</a><span>|</span></li>
												</c:if>
											</shiro:hasPermission>
											<shiro:hasPermission name="finance:orderAppeal:delete">
												<c:if test="${orderAppeal.status == 'new'}">
													<li><a href="javascript:deleteOrderAppeal('${orderAppeal.appealId}');" class="delete">删除</a></li>
												</c:if>
											</shiro:hasPermission>
										</ul>
									</div>
...
<script>
	secondLimitStr($('.finance-manage-wrap .list99'), '10');
	secondLimitStr($('.finance-manage-wrap .list100'), '10');
