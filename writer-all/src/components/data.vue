      ...
      <div class="resource-list">
        <ul class="clearfix">
          <li v-for="list in menuList" :class="[list.permission == 0 ? 'not development' : 'normal']" @click="router(list.name)">
            <div class="li-left">
              <img :src="'static/pub/images/data_' + list.id + '_x.png'" v-if="list.permission == 0">
              <img :src="'static/pub/images/data_' + list.id + '.png'" v-if="list.permission == 1">
            </div>
            <div class="li-right">
              <h3>{{list.name}}</h3>
              <p>{{list.explain}}</p>
      ...
      getMenuList() {
        ...
        $.ajax({
          ...
          success: function (data) {
            if(data.code == 0) {
              self.menuList = data.data[2].class_data;
              self.name = data.data[2].class_name;
            }
          }
        })
      },
