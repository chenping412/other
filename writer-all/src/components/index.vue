<template>
  <div id="index">
    <div class="aside-menu">
      <ul>
        <li v-for="(item,$index) in classList" @click="changeMenu($index)">
          <a :class="{'active':menuIndex==$index}" href="javascript:;">{{item.class_name}}</a>
        </li>
      </ul>
    </div>
          ...
          <div class="resource-list">
            <ul class="clearfix">
              <li v-for="list in menuList" :class="[list.permission == 0 ? 'not development' : 'normal']"
                ...
                <div class="li-left">
                  <img :src="'static/pub/images/img_'+ classId + '_' + list.id + '_x.png'" v-if="list.permission == 0">
                  <img :src="'static/pub/images/img_'+ classId + '_' + list.id + '.png'" v-if="list.permission == 1">
                </div>
                <div class="li-right">
                  <h3>{{list.name}}</h3>
                  <p>{{list.explain}}</p>
  ...
  export default {
    ...
    methods: {
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data.code == 0) {
              ...
              if(!self.classList[self.menuIndex]){
                self.menuIndex=0;
              }
              self.menuList=self.classList[self.menuIndex].class_data;
              self.name=self.classList[self.menuIndex].class_name;
              self.classId=self.classList[self.menuIndex].class_id;
            }
          },
      ...
      changeMenu(index){
        ...
        this.menuList=this.classList[this.menuIndex].class_data;
        this.name=this.classList[this.menuIndex].class_name;
        this.classId=this.classList[this.menuIndex].class_id;
        this.$router.push({
          path: '/index?index='+this.menuIndex
        })
      },
      router(name,permission) {
        ...
        var self = this;
        if(name == "行业简报") {
          self.$router.push('/classify?index='+self.menuIndex);
        }
        if(name == "行业快报") {
          self.$router.push('/ai-pager?index='+self.menuIndex);
        }
        if (name == "股票写作") {
          self.$router.push('/stock-comment?index='+self.menuIndex);
        }
        if (name == "财经写作") {
          self.$router.push('/write-robot?index='+self.menuIndex);
        }
      }
    }
  }
...
<style>
  ...
  #index .aside-menu ul li a.active {
    color: #0874ea;
    background-color: #f5f5f5;
