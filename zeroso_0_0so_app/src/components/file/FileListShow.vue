...
<script>
    export default {
        props: ['module_id','module_name','subsystem','showDelete','fileList','updateFileList','isMinio'],
        data() {
            return {
                fileListShow:[],
                previewSrcList:[],
        ...
        methods: {
            init(){
                if(this.fileList){
                    ...
                    if(this.isMinio){
                        this.getMinioFileListData();
                    }else {
                        this.getFileListData();
                    }
                }else {
                    this.setFileListShow([])
                }
            },
            getFileListData() {
                let self = this;
            ...
            setFileListShow(list){
                ...
                for(let i=0;i<list.length;i++){
                    if(self.isMinio){
                        list[i].url = self.$api.oss_file + '/' + list[i].entity_uid + '/' + list[i].id;
                        list[i].isImage = list[i].content_type.indexOf('image')>-1;
                    }else {
                    ...
                    self.$confirm('确定删除该附件吗？删除后将不可恢复', '提示', {
                        ...
                        callback: function (action) {
                            if (action == 'confirm') {
                                self.loading = true;
                                self.$http({
                                    url:self.isMinio ? self.$api.oss_file + '/' + item.entity_uid + '/' + item.id : self.$api.file_files_list+'/'+item.id,
                                    type: 'DELETE',
                                    success () {
                                        self.loading = false;
                                        self.$message({
