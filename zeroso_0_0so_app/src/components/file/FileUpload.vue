<template>
    <div class="file-upload-component">
        <el-upload
                ...
                :limit="limit">
            <el-button slot="trigger" size="mini" type="primary" :disabled="disabled || loading" :loading="loading">{{loading ? '上传中' : autoUpload ? '上传' : '选择文件'}}</el-button>
        </el-upload>
    </div>
</template>
...
<script>
    export default {
        props: ['module_id', 'module_name', 'subsystem','isOnlyOne' ,'disabled', 'autoUpload', 'showFileList', 'selectFileList', 'updateFileList', 'fileNamePrefix','accept','maxSizeMB','multiple','limit','isMinio'],
        data() {
            return {
                actionUrl: '',
                fileList: [],
        ...
        methods: {
            init(module_id) {
                let self = this;
                let url='';
                this.$emit('update:module_id', module_id);
                if(self.isMinio){
                    url = self.$api.oss_file + '/' + self.subsystem;
                }else {
                    url = (self.isOnlyOne ? self.$api.file_unique_file : self.$api.file_files_list) + '?subsystem=' + self.subsystem + '&module_name=' + self.module_name + '&module_id=' + module_id;
                }
            ...
            fileChange(file, fileList) {
                if (this.fileNamePrefix) {
                    for (let i = 0; i < fileList.length; i++) {
                        let fileName = this.fileNamePrefix + (this.isOnlyOne ? '' : this.isMinio ? i + new Date().getTime() : this.alreadyUpdateFileListLength + i);
            ...
            fileUpdateRequest(params) {
                ...
                if (this.fileNamePrefix && this.autoUpload) {
                    let fileName = this.fileNamePrefix + (this.isOnlyOne ? '' : this.isMinio ? new Date().getTime() : this.alreadyUpdateFileListLength+1);
            ...
            uploadSuccess(data) {
                ...
                if(this.isMinio){
                    if (data.msg) {
                        updateFileList = updateFileList.concat(data.msg);
                    }
                }else {
