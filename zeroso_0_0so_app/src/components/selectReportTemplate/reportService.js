...
let reportService = {
    //在订单页面组织零搜供应链与联营公司的合同数据
    set0soInquiryReportInOrderPage(template, data, callback) {
        let self = this;
        ...
        let company39567Index=0;
        order.order_nodes.forEach((line,index)=>{
           if(line.account_company_id === 39567){
               company39567Index = index;
           }
        });
        //甲方
        let partyA = {};
        ...
        //乙方
        let partyB = order.order_nodes[company39567Index+1];

        //组装产品行数据
        let order_lines = [];
        if (order.order_products) {
            order.order_products.forEach((line) => {
                if (line.is_active) {
                    order_lines.push({
                        without_tariff_purchase_unit_price: line.order_lines[company39567Index].price_unit,//  未税单价
                        with_tariff_purchase_unit_price: line.order_lines[company39567Index].price_taxed_unit,// 含税产品单价
                        with_tariff_total_price: line.order_lines[company39567Index].price_amount,// 含税总价
                        shipment_period_week: line.order_lines[company39567Index].shipment_period,
                        qty: line.qty,
                        product: {
                            ...
                            product_order_no: line.product.product_order_no,
                            product_parameter: line.product.product_parameter,
                            product_trade_name: {
                                product_cn_name: line.product.product_trade_name ? line.product.product_trade_name.product_cn_name : ''
                            },
                        }
                    })
                }
        ...
        let domestic_total_price = order.order_nodes[company39567Index].total_amount;//人民币总价
        result.data = {
            name: order.name,//合同编号
            order_name: order.name,//合同编号
            ...
            deliver_type: order.order_nodes[company39567Index].delivery_method === 'domestic_delivery' ? 3 : order.order_nodes[company39567Index].delivery_method === 'foreign_delivery' ? 2 : 1, //发货方式
            ...
            brand: order.brand,
            currency: order.order_nodes[company39567Index].currency,
    ...
    //组织由零搜报价给联谊公司的数据
    set0soInquiryReport(template, data, reportType, callback) {
        let self = this;
        let currentTime = Vue.prototype.$moment().format('YYYY-MM-DD HH:mm');
            ...
            inquiryData.products.forEach((item) => {
                if (!item.mark_as_deleted && item.product_quote_state !== 'INVALID' && item.product_quote_state !== 'REPLACED') {
                    ...
                    if (isAlreadyExpenseApproved) {
                        ...
                        if (inquiryData.procurement_price.additional_charges.extra_charges.value_add_cny_tax && inquiryData.procurement_price.additional_charges.extra_charges.value_add_cny_tax !== 0) {
                            let productPriceWithTariffWithTax = (Number(item.prodcut_quotation.procurement.total_cny_price_with_tariff) / item.prodcut_quotation.quantity * 1.13).myRound(2);
                            without_tariff_purchase_unit_price = setInquiryReportFormatMoney(item.prodcut_quotation.procurement.cny_price_with_tariff);
                            ...
                            without_tariff_purchase_unit_price = setInquiryReportFormatMoney(item.prodcut_quotation.procurement.cny_price_with_tariff);
                            with_tariff_purchase_unit_price = setInquiryReportFormatMoney(Number(item.prodcut_quotation.procurement.total_cny_price_with_tariff) / item.prodcut_quotation.quantity);
                            with_tariff_total_price = setInquiryReportFormatMoney(item.prodcut_quotation.procurement.total_cny_price_with_tariff);
                        }
                    } else {
                        without_tariff_purchase_unit_price = setInquiryReportFormatMoney(item.prodcut_quotation.procurement.price);
                        with_tariff_purchase_unit_price = setInquiryReportFormatMoney(Number(item.prodcut_quotation.procurement.total_price_with_tariff) / item.prodcut_quotation.quantity);
                        with_tariff_total_price = setInquiryReportFormatMoney(item.prodcut_quotation.procurement.total_price_with_tariff);
                    }

                    let tariff_ratio = '待确认';
                    if (item.prodcut_quotation.procurement.tariff || item.prodcut_quotation.procurement.tariff === 0) {
                        tariff_ratio = (Number(item.prodcut_quotation.procurement.tariff) + (Number(item.prodcut_quotation.procurement.extra_import_tariff) || 0)) + '%';
                    }
        ...
        //生成订单打印数据
        let firstPayRate = 0.3;
        let domestic_total_price = inquiryData.procurement_price.grand_cny_price;//人民币总价,只有核准税费之后才有
        ...
        if (isAlreadyExpenseApproved) {
            product_total_price = setInquiryReportFormatMoney(inquiryData.procurement_price.total_cny_products_price);
            attached_total_fee = setInquiryReportFormatMoney(0);
            value_added_tax = setInquiryReportFormatMoney(inquiryData.procurement_price.additional_charges.extra_charges.value_add_cny_tax);

            foreign_storage_opr_fee = setInquiryReportFormatMoney(0);
            domestic_storage_opr_fee = setInquiryReportFormatMoney(0);
        } else {
            product_total_price = setInquiryReportFormatMoney(inquiryData.procurement_price.total_products_price);
            attached_total_fee = ((inquiryData.procurement_price.additional_charges.product_item_based_fees.handling_and_packing_fees || 0)
                    + (inquiryData.procurement_price.additional_charges.product_item_based_fees.overseas_customs_fees || 0)
                    + (inquiryData.procurement_price.additional_charges.product_item_based_fees.domestic_customs_clearance_fees || 0)
                    + (inquiryData.procurement_price.additional_charges.product_weight_based_fees.supplier_pick_up_fees || 0)
                    + (inquiryData.procurement_price.additional_charges.product_weight_based_fees.international_logistics_fees || 0)
                    + (inquiryData.procurement_price.additional_charges.product_weight_based_fees.domestic_logistics_fees || 0)
                    + (inquiryData.procurement_price.additional_charges.extra_charges.insurance_fees || 0)
                    + (inquiryData.procurement_price.additional_charges.extra_charges.foreign_exchange_fees || 0)) || null;
            attached_total_fee = setInquiryReportFormatMoney(attached_total_fee)
            value_added_tax = setInquiryReportFormatMoney(inquiryData.procurement_price.additional_charges.extra_charges.value_add_tax);

            foreign_storage_opr_fee = setInquiryReportFormatMoney(inquiryData.procurement_price.additional_charges.optional_service_fees.oversee_storage_service_fee)
            domestic_storage_opr_fee = setInquiryReportFormatMoney(inquiryData.procurement_price.additional_charges.optional_service_fees.domestic_storage_service_fee)
        }
    ...
    setInquiryEmail(template, data, selectProductList, callback) {
        ...
        let result = {
            id: template.id.toString(),
            file_name: 'inquiry_' + inquiryData.id,
        };
        let currentCompany = JSON.parse(JSON.stringify(store.state.currentCompany));
        currentCompany.account_user_id = currentCompany.user_id;
        ...
        //供应商
        let partyB = {
            account_company_id: inquiryData.supplier_company ? inquiryData.supplier_company.account_company_id : inquiryData.inquiry_supplier_company ? inquiryData.inquiry_supplier_company.account_company_id : '',
            account_company: inquiryData.supplier_company || inquiryData.inquiry_supplier_company || '',
            account_user_id: '',
            account_user: {},
            address: {},
            email: '',
        ...
        if (selectProductList) {
            selectProductList.forEach((item) => {
                order_lines.push({
                    qty: item.prodcut_quotation ? item.prodcut_quotation.quantity : '',
                    product: {
                        brand_id: inquiryData.brand.id,
                        brand_name: inquiryData.brand.name,
                        brand: {
