    ...
    export default {
        ...
        methods: {
            init(allowFilterFieldsFast, allowFilterFieldsMore) {
                ...
                console.log(this.allowFilterFieldsFastCopy)
            },
            initSet(allowFilterList, filterObj) {
                allowFilterList = JSON.parse(JSON.stringify(allowFilterList));
                let arrList = [];
                    ...
                    if (item[item.field_name] || item[item.field_name] === false) {
                        if (JSON.stringify(item[item.field_name]) !== "{}") {
                            filter.push([
                                item.field_name,
                                item.operator || '=',
                                item[item.field_name].id === '无' ? '无' : typeof(item[item.field_name]) === "string" ? item[item.field_name].replace(/(^\s*)|(\s*$)/g, "") : item[item.field_name]
                            ])
                        }
                    }
                });
