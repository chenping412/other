...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        setCurrentCompanyList(state, data) {
            ...
            // 林露 310 ；郑少贤 441 ; 明灿 448 ；彭明霞 380; 陈弢Virgil 342; 胡飞龙 1430;
            let canLoginLoginId = [310, 441, 448, 380, 342, 132, 1430];
        ...
        setProductQuotationComment(state, data) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (data[i].children_map && data[i].children_map.comment_no) {
                    for (let k = 0; k < data[i].children_map.comment_no.length; k++) {
                        if (!data[i].deleted) {
                            productQuotationCommentAll[data[i].value].children.push({
                                label: data[i].children_map.comment_no[k].label,
                                code: Number(data[i].children_map.comment_no[k].value),
                            });
                            if (!data[i].children_map.comment_no[k].deleted) {
