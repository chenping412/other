<template>
    <div class="partner-detail-brands clearfix" v-loading="loading">
        <el-table :data="listData" border>
            <el-table-column prop="name" label="品牌" width="400px">
                <template slot-scope="scope">
                    <router-link class="blue" target="_blank" :to="'/product/brand/detail?id='+scope.row.brand.id" v-if="scope.row.brand">
            ...
            <el-table-column prop="comment" label="品牌备注">
                <template slot-scope="scope">
                    <pre v-if="scope.row.brand">{{scope.row.brand.comment}}</pre>
                </template>
            </el-table-column>
