<template>
    <div>
        <el-button type="primary" plain @click="edit">状态修改</el-button>

        <el-dialog :title="actionDialogTitle || '状态修改'" :visible.sync="dialogShow" width="550px" :close-on-click-modal="false" append-to-body  v-loading="loading">
...
<script>
    export default {
        props:['queryId','selectUserForm','actionDialogTitle'],
        data() {
            return {
                loading:false,
                dialogShow:false,
            ...
            clickSaveDate() {
                ...
                self.$refs.form.validate(function (valid) {
                    if (valid) {
                        self.loading = true;
                        self.$http({
                            url: self.$api.partner_companies_v5 + '/' + self.queryId + '/partner_users' + '/' + self.selectUserForm.id + '/status_action',
                            type:'PUT',
                            requestBody:true,
                            data: self.addForm,
