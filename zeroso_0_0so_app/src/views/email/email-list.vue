...
<!--                 <el-table-column type="selection" width="55"></el-table-column>-->
                ...
                <el-table-column prop="mail_from" label="发件人" show-overflow-tooltip>
                    <template slot-scope="scope">
                    <span v-if="scope.row.partner_company_from">
                        {{scope.row.partner_company_from.company_name + '，' + scope.row.partner_company_from.name}}
                    </span>
                    <span v-else>{{scope.row.mail_from}}</span>
                    </template>
                </el-table-column>
                <el-table-column label="收件人" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <span v-if="scope.row.partner_company_to">
                            {{scope.row.partner_company_to.company_name + '，' + scope.row.partner_company_to.name}}
                        </span>
                        <template v-else>
                            <span v-for="(item,$index) in scope.row.mail_to" :key="$index">{{$index!==0 ? '，' : ''}}{{item}}</span>
                        </template>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="主题" prop="message_subject" show-overflow-tooltip></el-table-column>
                ...
                <el-table-column label="关联单" width="180">
                    <template slot-scope="scope">
                        ...
                        <p v-if="scope.row.module_name==='quote_inquiry' && scope.row.module_id">询报价单：{{scope.row.module_id}}</p>
                    </template>
                </el-table-column>
