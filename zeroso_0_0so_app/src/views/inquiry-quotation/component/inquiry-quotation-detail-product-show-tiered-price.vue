<template>
    <el-popover trigger="click" placement="left" width="350" @show="getProductPriceList">
        ...
        <div v-loading="loading">
            ...
            <p v-if="priceDescriptionAll">报价备注：{{priceDescriptionAll}}</p>
        </div>
    </el-popover>
</template>
...
<script>
    ...
    export default {
        props: ['productLine', 'queryId','isAlreadyExpenseApproved','detailForm','productMinQty','priceDescription'],
        data() {
            return {
                loading: false,
                ...
                if(self.productLine.product_id && self.productLine.prodcut_quotation.quote_supplier_id){
                    ...
                    self.$http({
                        ...
                        success(data) {
                            ...
                            self.priceDescriptionAll = null;
                            for(let k=0;k<productPriceList.length;k++){
                                if(productPriceList[k].price_description){
                                    self.priceDescriptionAll = productPriceList[k].price_description;
                                    break
                                }
                            }
                            self.$emit('update:priceDescription',self.priceDescriptionAll);
                        }
                    });
                }
            },
