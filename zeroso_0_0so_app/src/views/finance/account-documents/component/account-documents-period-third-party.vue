        ...
        methods: {
            ...
            clickActionButton(item) {
                ...
                if (item.button_type === 'exit') {
                    ...
                    if(item.is_confirm){
                        self.$confirm(item.confirm_comment||'是否确定执行？', '提示', {
                            confirmButtonText: '确认',
                            cancelButtonText: '取消',
                            type: 'warning',
                            callback: function (action) {
                                if (action === 'confirm') {
                                    self.clickActionButtonSubmit(item);
                                }
                            }
                        })
                    }else {
            ...
            clickActionButtonSubmit(item) {
                let self = this;
                self.loading = true;
                self.$http({
                    url: item.action_url,
                    type: item.action_method ? item.action_method.toUpperCase() : null,
                    data:{
