                ...
                <el-table :data="formData.listData" border v-loading="loading" size="mini">
                    ...
                    <el-table-column label="型号/订货号" v-if="isImport">
                        <template slot-scope="scope">
                            <p class="nowrap" :title="scope.row.product_name">{{scope.row.product_name}}</p>
                            <p class="nowrap" :title="scope.row.product_order_no">{{scope.row.product_order_no}}</p>
                        </template>
                    </el-table-column>
            ...
            <el-row>
                    ...
                    <el-form label-width="180px" class="padding-top-10">
                        ...
                        <el-form-item label="供货商提货费：">
                            <div class="form-border-bottom">
                                <span class="red" v-if="formData.custom_supply_take_delivery_fee">
                                        （
                                        自定义：{{formData.custom_supply_take_delivery_fee | formatMoney('EUR') | inquiryPriceNormalText}}，
                                        系统：{{formData.procurement_price.supplier_pick_up_fees | formatMoney('EUR') | inquiryPriceNormalText}}
                                        ）
                                    </span>
                                <span>
                                    {{(
                                        Number(formData.custom_supply_take_delivery_fee) && Number(formData.custom_supply_take_delivery_fee) > formData.procurement_price.supplier_pick_up_fees ?
                                        Number(formData.custom_supply_take_delivery_fee) : formData.procurement_price.supplier_pick_up_fees
                ...
                <el-col :span="10">
                    <el-form :model="formData" :rules="formDataRule" ref="formDataRef" label-width="150px" class="padding-top-10">
                        ...
                        <el-form-item label="自定义供应商提货费">
                            <InputNumber
                                    :value.sync="formData.custom_supply_take_delivery_fee"
                                    :minus="true"
                                    @change="saveLocalStorage">
                                <template slot="append">€</template>
                            </InputNumber>
                        ...
                        <div class="red" style="padding-left: 150px;" v-if="isImport">
                            ...
                            <p style="margin-bottom: 5px;">1，产品的面价、折扣(或折扣系列)、毛重、关税、额外关税，供应商付款方式，这些基础数据均为当前最新数据，并不是给此询报价单报价时的数据，因为这些数据有可能会修改，因而无法得知当时给询报价单报价时的数据</p>
        ...
        data() {
            return {
                ...
                formData: {
                    ...
                    discount_amount: '',
                    custom_supply_take_delivery_fee: '',
                    listData: [],
                    procurement_price: {},
                    sales_price: {},
                },
        ...
        methods: {
            clickShow(priceCalculatorData) {
                ...
                if(this.autoSave){
                    ...
                    this.formData.custom_supply_take_delivery_fee = priceCalculatorData.custom_supply_take_delivery_fee;
            ...
            attachedIntermediateFee(formData) {
                let supplier_pick_up_fees = 0;
                if(Number(formData.custom_supply_take_delivery_fee) && Number(formData.custom_supply_take_delivery_fee) > formData.procurement_price.supplier_pick_up_fees){
                    supplier_pick_up_fees = Number(formData.custom_supply_take_delivery_fee)
                }else {
                    supplier_pick_up_fees = formData.procurement_price.supplier_pick_up_fees
                }
