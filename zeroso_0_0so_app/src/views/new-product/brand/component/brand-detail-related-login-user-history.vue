        ...
        <el-dialog title="跟进人变更记录" :visible.sync="dialogShow" width="90%" top="30px" :close-on-click-modal="false" v-loading="loading">
            ...
            <el-table :data="listData" border height="600px">
                ...
                <el-table-column label="操作人">
                    <template slot-scope="scope">
                        <span v-if="scope.row.operator_user">{{scope.row.operator_user.nick_name}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="country" label="原跟进人">
                    <template slot-scope="scope">
                        <span v-if="scope.row.origin_follower_user">{{scope.row.origin_follower_user.nick_name}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="country" label="新跟进人">
                    <template slot-scope="scope">
                        <span v-if="scope.row.new_follower_user">{{scope.row.new_follower_user.nick_name}}</span>
                    </template>
                </el-table-column>
            </el-table>
    ...
    export default {
        ...
        methods: {
            ...
            getListData() {
                ...
                self.loading = true;
                self.$http({
                    url: self.$api.product_products_brands + '/' + self.queryId + '/follower/get_history',
                    type: 'GET',
                    data: {
                        page: self.pageNum,
