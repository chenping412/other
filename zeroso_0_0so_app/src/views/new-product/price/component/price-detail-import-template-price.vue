    ...
    <div v-loading="loading">
        <div>
            <FileListShow
                    ...
                    module_name="price_template_file2"
        ...
        methods: {
            ...
            verifyTemplateSubmit(isClearVerify){
                ...
                if(self.detailForm.uid){
                    self.loading = true;
                    self.$http({
                        url: self.$api.new_product_price_product_template + '/' + self.detailForm.uid + (isClearVerify ? '/clear_verify' : '/verify'),
                        success (data) {
                            self.loading = false;
            ...
            getTemplateErrataData() {
                ...
                if(self.detailForm.uid) {
                    self.loading = true;
                    self.$http({
                        url: self.$api.new_product_price_product_template + '/errata/' + self.detailForm.uid,
                        data: {
                            page: self.pageNum,
