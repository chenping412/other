<template>
    <div class="brand-detail-import-price" v-loading="loading">
        ...
        <el-table :data="listData" border>
            ...
            <el-table-column prop="file_size" label="文件大小(KB)">
                <template slot-scope="scope" v-if="scope.row.file_size">
                    {{scope.row.file_size}}KB
                </template>
            </el-table-column>
            <el-table-column label="供应商">
                <template slot-scope="scope" v-if="scope.row.supplier_company">
                    {{scope.row.supplier_company.company_name}}
                </template>
            </el-table-column>
            <el-table-column prop="count" label="价格表产品数"></el-table-column>
            <el-table-column prop="count_before_import" label="导入前产品数"></el-table-column>
            <el-table-column prop="count_after_import" label="导入后产品数"></el-table-column>
            <el-table-column prop="count_of_error" label="导入失败产品数"></el-table-column>
            <el-table-column label="导入记录">
                <template slot-scope="scope">
                    <el-button type="text" v-if="scope.row.count_of_error" size="mini"
                               @click="openErrorProduct(scope.row.id)">查看出错产品记录
                    </el-button>
                </template>
            </el-table-column>
            <el-table-column label="跟进人">
                <template slot-scope="scope" v-if="scope.row.operator_user && scope.row.operator_user.name">
                    {{scope.row.operator_user.name}}
                </template>
            </el-table-column>
            ...
            <el-form label-width="100px">
                ...
                <el-form-item label="文件" required>
                    ...
                    <FileUpload
                            ...
                            :file_namePrefix="file_namePrefix"
                            :multiple="false"
                            :limit="1"
                            :accept="['text/plain']"
                            module_id="0"
        ...
        <el-dialog title="查看出错产品记录" :visible.sync="errorProductDialogShow" width="1200px">
            ...
            <el-table :data="errorProductList" border height="550px" v-loading="loading">
                <el-table-column type="index" label="序号" width="55" align="center"></el-table-column>
                <el-table-column prop="line_num" label="出错行" width="100"></el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            supplierChange(){
                let self = this;
                self.file_namePrefix = 'Price-' + self.brandDetailForm.name + '-' + self.selectSupplierId + '-' + self.$moment().format('YYYYMMDDHHmms');
            },


            exportPriceSubmit(row) {
                let self = this;
                self.$confirm('注意：请务必谨慎执行该操作，当前选择的推荐供应商是 <b class="red">' + (row.supplier_company?row.supplier_company.company_name:'') + '</b>，在执行导入之前，请确认价格表是否是该供应商提供的，如果不是，必须先将推荐供应商修改为提供价格表的供应商，否则执行导入后的价格将是错误价格！', '提示', {
