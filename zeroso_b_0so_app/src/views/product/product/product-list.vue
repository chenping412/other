            ...
            <el-table :data="listData" @row-click="openDetail" border v-loading="loading">
                ...
                <el-table-column prop="state" label="状态">
                    <template slot-scope="scope">
                       <el-tag type="warning">{{scope.row.state | productState | defaultStr}}</el-tag>

                        <el-tag type="info" effect="dark" v-if="scope.row.is_valid === 0">已标记无效</el-tag>
                        <el-tag type="info" effect="dark" v-if="scope.row.is_replaced === 1">已标记被替代</el-tag>
                        <el-tag type="danger" effect="dark" v-if="scope.row.is_3C === 1">已标记3C</el-tag>
                    </template>
                </el-table-column>
            </el-table>
        </div>
