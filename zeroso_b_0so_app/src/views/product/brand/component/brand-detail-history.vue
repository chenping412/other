<template>
    <div class="brand-detail-history">
        <BrandLever class="brand-lever" :state="state" @click.native="init"></BrandLever>
        <i class="el-icon-s-unfold" style="font-size: 30px;" @click="init"></i>

        <el-dialog :title="'品牌名:'+name" :visible.sync="dialogShow" width="1200px" fullscreen :close-on-click-modal="false" v-loading="loading">
            <div class="clearfix">
                <el-pagination
                        class="right"
            ...
            <el-table :data="listData" border height="550px">
                ...
                <el-table-column label="原统计开始时间">
                    <template slot-scope="scope">
                        {{scope.row.origin_state_start_time | zerosoTimeFormat}}
                    </template>
                </el-table-column>
                <el-table-column prop="state" label="新统计开始时间">
                    <template slot-scope="scope">
                        {{scope.row.new_state_start_time | zerosoTimeFormat}}
                    </template>
                </el-table-column>
                <el-table-column prop="create_order_times" label="询价次数" align="right"></el-table-column>
                <el-table-column prop="quoted_order_times" label="报价次数" align="right"></el-table-column>
                <el-table-column prop="performed_order_times" label="成单次数" align="right"></el-table-column>
                <el-table-column label="报价率" align="right">
                    <template slot-scope="scope">
                        {{scope.row.quoted_order_rate | tenthsToPercent}}
                    </template>
                </el-table-column>
                <el-table-column prop="state" label="成单率" align="right">
                    <template slot-scope="scope">
                        {{scope.row.performed_order_rate | tenthsToPercent}}
                    </template>
                </el-table-column>
                <el-table-column prop="comment" label="原因"></el-table-column>
            </el-table>
