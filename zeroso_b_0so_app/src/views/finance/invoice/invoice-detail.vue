    ...
    export default {
        ...
        methods: {
                ...
                self.$http({
                    ...
                    success: function (data) {
                        if (data.err_code.length === 0 && data.msg) {
                            ...
                            if (currentPartner.follower) {
                                self.detailForm.customer_user_name = currentPartner.follower.name;
                            }
                        }
                    }
                })
            ...
            supplierCompanyIdChange(company) {
                ...
                if (!company || !company.account_company_id) return false
                self.$http({
                    url: self.$api.partner_companies_detail + '/' + company,
                    success: function (data) {
                        if (data.err_code.length === 0 && data.msg) {
                            let currentPartner = data.msg;
                            ...
                            if (currentPartner.follower) {
                                self.detailForm.supplier_user_name = currentPartner.follower.name;
                            }
                        }
                    }
                })
